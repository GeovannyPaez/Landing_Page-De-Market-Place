import { navigation, enventScrollHorizontal, scrollUp } from "./srcs/navigaton.mjs";
import {
  getProductAlls,
  getProductsOfCart,
} from "../src/srcs/classOfProducts.mjs";

export let productosAll = [];
//selecion de elementos
const imgLogoNavBar=document.querySelector('.logo');
...
export const containerCategories = document.querySelector(
  ".categories-container"
);
export const sectionProdutsAll = document.querySelector(".productosAll");
export const sectionContainerAllProducts = document.querySelector(
  ".container-productsAll"
);
...
const asideAlertAddProdutToCar = document.querySelector(
  ".container-alert-AddToCar"
);
 export const btnScrollUp=document.querySelector('.container-ArrowUp');
...
const toggleMenuMobile = () => {
  ...
  let isOpenListProducDetails = !listaProductsDetail.classList.contains(
    "inactive_listProductDetail"
  );
...
const toggleBtnScrollUp=()=>{
    const {scrollTop} = document.documentElement;
    scrollTop>=550
    ?btnScrollUp.classList.remove('inactive')
    :btnScrollUp.classList.add('inactive')
}
...
imgLogoNavBar.addEventListener('click',()=>{
    location.hash= 'home'
})
...
btnScrollUp.addEventListener('click',scrollUp,false)
...
window.addEventListener('scroll',toggleBtnScrollUp);

//validacion service worker
