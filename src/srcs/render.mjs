...
import { observer } from "./intersectuionObserver.js";
...
export const renderProducts = (container, productos,lazziLoading) => {
  ...
  productos.forEach((product) => {
    ...
    //observar card product

    const urlImg = product.images[0];
    
    const containerImg= document.createElement('div')
    containerImg.classList.add('imageContainer');
    const imgProduct = createElement("img");
    imgProduct.classList.add("imgProductCard");
    imgProduct.alt = product.title;
    imgProduct.setAttribute((lazziLoading?'url-img':'src'), urlImg);
    if(lazziLoading){
      imgProduct.classList.add("inactive-product--card");
    ...
    cardProduct.insertAdjacentElement("beforeend",containerImg);
    cardProduct.insertAdjacentElement("beforeend", containerInfo);

    listProducts.push(cardProduct);
...
export const renderCategoriesNavbar = (container, categories) => {
  ...
  all.addEventListener("click", (e) => {
    if (container.classList.value == "category-navbar--mobile") {
      ...
      menu.classList.add("inactive_mobile-menu");
